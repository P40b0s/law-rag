...
use rag_core::{ChunkedText, Encoder};
pub struct Splitter<'m, E: Encoder>
{
    encoder: &'m E,
}
impl<'m, E: Encoder> Splitter<'m, E>
{
    pub async fn new(encoder: &'m E) -> Result<Self>
    {
        Ok(Self
        {
           encoder
    ...
    pub async fn split_text(&self, text: &str) -> Result<Vec<ChunkedText>> {
        ...
        let encoding = self.encoder.encode(text, false)
        ...
        let total_chunks = (total_tokens + self.encoder.max_tokens() - 1) / self.encoder.max_tokens();
        ...
        while start < total_tokens && chunk_index < max_chunks {
            ...
            let mut end = std::cmp::min(start + self.encoder.max_tokens(), total_tokens);
            ...
            if end < total_tokens {
                ...
                if let Some(optimal_end) = self.find_optimal_cutoff(&tokens, start, end, self.encoder.tokenizer()).await? {
                    end = optimal_end;
            ...
            let chunk_text = self.encoder.decode(chunk_tokens, false)
            ...
            let next_start = if end > self.encoder.overlap_tokens() {
                end - self.encoder.overlap_tokens()
            } else {
                end
            };
    ...
    let search_start = start + (self.encoder.max_tokens() * 3 / 4);  // 75%
    ...
    if let Some((position, priority)) = best_boundary {
        ...
        let min_position = start + self.encoder.max_tokens() / 3;
    ...
    if let Some(word_boundary) = self.find_word_boundary_fast(&text_fragment, start).await? {
        ...
        if position > start + self.encoder.max_tokens() / 2 {
