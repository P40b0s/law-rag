...
export default function useModelState()
{
    ...
    {
        const new_state = await http_sevice.model_state_service.load_embedding_models();
        if (new_state)
            set_state(new_state)
    }
    const unload_embedding_models = async () =>
    {
        const new_state = await http_sevice.model_state_service.unload_embedding_models();
        if (new_state)
            set_state(new_state)
    }
    const load_generator_model = async () =>
    {
        const new_state = await http_sevice.model_state_service.load_generator_model();
        if (new_state)
            set_state(new_state)
    }
    const unload_generator_model = async () =>
    {
        const new_state = await http_sevice.model_state_service.unload_generator_model();
        if (new_state)
            set_state(new_state)
    }
    
    return {set_state, get_state, load_embedding_models, unload_embedding_models, load_generator_model, unload_generator_model}
}